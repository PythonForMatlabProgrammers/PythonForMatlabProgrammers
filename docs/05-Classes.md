...
class MyClass:
        ...
        print(StringToPrint)
```
You'll need to save the above into a .py file.  You can choose any name for the file; let's use simple.py.  Back in the Python console window, we can type the following:
```python
  >>> import simple
  >>> obj = simple.MyClass()
  >>> obj.x
